...
router.post('/', authenticateToken, authorizeRole(['Agent', 'Admin']), (req, res) => {
    const { title, description, destination, price, duration, image_url, available_slots } = req.body;
    ...
    try {
      ...
      INSERT INTO packages (agent_id, title, description, destination, price, duration, image_url, available_slots)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(req.user.id, title, description, destination, price, duration, image_url, available_slots);
...
router.put('/:id', authenticateToken, authorizeRole(['Agent', 'Admin']), (req, res) => {
    ...
    try {
        // Check ownership if Agent (optional, but good practice)
      ...
      WHERE id = ?
    `).run(title, description, destination, price, duration, image_url, available_slots, packageId);
